                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500">{{ $run->run_month->format('F Y') }} - {{ $run->fiscalYear?->label ?? 'N/A' }}</p>
                        </div>
                    </div>
                    <div>
                        @php
                            $statusColors = ['Calculated' => 'bg-yellow-100 text-yellow-800', 'Approved' => 'bg-green-100 text-green-800', 'Locked' => 'bg-purple-100 text-purple-800'];
                            $color = $statusColors[$run->status] ?? 'bg-gray-100 text-gray-800';
                        @endphp
                        <span class="px-4 py-2 text-sm font-bold rounded-full {{ $color }}">{{ $run->status }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <div class="flex flex-wrap gap-3 mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
                    ...
                    @elseif($run->status === 'Locked')
                        <div class="px-5 py-2.5 text-sm font-semibold text-purple-700 bg-purple-100 rounded-lg flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
                    ...
                    <div class="bg-red-50 rounded-xl p-4 border border-red-200">
                        ...
                        <p class="text-2xl font-bold text-red-800 mt-1">{{ number_format($totals['deductions'], 2) }}</p>
                    </div>
                    ...
                    <div class="bg-amber-50 rounded-xl p-4 border border-amber-200">
                        ...
                        <p class="text-2xl font-bold text-amber-800 mt-1">{{ $totals['count'] > 0 ? number_format($totals['net'] / $totals['count'], 2) : '0.00' }}</p>
                    </div>
                </div>
                    ...
                    <div class="border border-gray-200 rounded-lg overflow-hidden">
                        ...
                        <button onclick="toggleBreakdown({{ $i }})"
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <span class="text-sm font-semibold text-gray-800">{{ $emp['employee_name'] }}</span>
                            ...
                            <div class="flex items-center gap-6">
                                <div class="text-right">
                                    ...
                                    <p class="text-sm font-semibold text-green-700">{{ number_format($emp['gross'], 2) }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-xs text-gray-500">Deductions</p>
                                    <p class="text-sm font-semibold text-red-700">{{ number_format($emp['deductions'], 2) }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-xs text-gray-500">Net Pay</p>
                                    <p class="text-sm font-bold text-purple-700">{{ number_format($emp['net'], 2) }}</p>
                                </div>
                        ...
                        <div id="breakdown-{{ $i }}" class="hidden border-t border-gray-200">
                            <div class="p-4 bg-white">
                                ...
                                <div class="mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                                    ...
                                    <div class="grid grid-cols-5 gap-2 text-xs">
                                        <div class="text-center p-2 bg-green-50 rounded">
                                            <p class="font-bold text-green-700">{{ $emp['attendance_summary']['present'] }}</p>
                                            <p class="text-gray-500">Present</p>
                                        </div>
                                        <div class="text-center p-2 bg-blue-50 rounded">
                                            <p class="font-bold text-blue-700">{{ $emp['attendance_summary']['working_days'] }}</p>
                                            <p class="text-gray-500">Working Days</p>
                                        </div>
                                        <div class="text-center p-2 bg-red-50 rounded">
                                            <p class="font-bold text-red-700">{{ $emp['attendance_summary']['late_days'] ?? 0 }} days</p>
                                            <p class="text-gray-500">Late ({{ $emp['attendance_summary']['late_minutes'] }}m)</p>
                                        </div>
                                        <div class="text-center p-2 bg-purple-50 rounded">
                                            <p class="font-bold text-purple-700">{{ $emp['attendance_summary']['overtime_minutes'] }}m</p>
                                            <p class="text-gray-500">Overtime</p>
                                        </div>
                                        <div class="text-center p-2 bg-orange-50 rounded">
                                            <p class="font-bold text-orange-700">{{ $emp['attendance_summary']['half_days'] }}</p>
                                            <p class="text-gray-500">Half Days</p>
                                        </div>
                                        <div class="text-center p-2 bg-red-100 rounded">
                                            <p class="font-bold text-red-800">{{ $emp['attendance_summary']['absent'] }}</p>
                                ...
                                <p class="text-xs font-semibold text-gray-500 uppercase mb-2">Basic Salary: {{ number_format($emp['basic_salary'], 2) }}</p>
                                <table class="min-w-full divide-y divide-gray-100">
                                    <thead>
                                        <tr class="text-xs text-gray-500 uppercase">
                                            ...
                                            <th class="px-3 py-2 text-right font-semibold">Amount ({{ session('currency', 'BDT') }})</th>
                                        </tr>
                                    </thead>
                                        ...
                                        <tr class="text-sm">
                                            ...
                                            <td class="px-3 py-2">
                                                @if($comp['type'] === 'Earning')
                                                    <span class="text-green-700 bg-green-50 px-2 py-0.5 rounded text-xs font-medium">Earning</span>
                                                @else
                                                    <span class="text-red-700 bg-red-50 px-2 py-0.5 rounded text-xs font-medium">Deduction</span>
                                                @endif
                                            </td>
                                            ...
                                            <td class="px-3 py-2 text-gray-500 text-xs">{{ $comp['calc'] }}</td>
                                            <td class="px-3 py-2 text-right font-medium {{ $comp['type'] === 'Earning' ? 'text-green-700' : 'text-red-700' }}">
                                    ...
                                    <tfoot class="bg-gray-50 text-sm font-semibold">
                                        <tr>
                                            ...
                                            <td class="px-3 py-2 text-right text-purple-700">{{ number_format($emp['net'], 2) }}</td>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
            ...
            $(document).ready(function() {
                ...
                $(document).on('click', '#btnRecalculate', function() {
                    ...
                    Swal.fire({
                        ...
                        preConfirm: () => {
                            return $.ajax({
                                url: '{{ route("payroll-runs.recalculate", ":id") }}'.replace(':id', id),
                                type: 'POST',
                                headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
                                success: function(res) {
                                    if (res.status === 'success') {
                                        Swal.fire({ icon: 'success', title: 'Recalculated!', text: res.message });
                                        location.reload();
                                    } else {
                ...
                $(document).on('click', '#btnApprove', function() {
                    ...
                    Swal.fire({
                        ...
                        preConfirm: () => {
                            return $.ajax({
                                url: '{{ route("payroll-runs.approve", ":id") }}'.replace(':id', id),
                                type: 'POST',
                                headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
                                success: function(res) {
                                    if (res.status === 'success') {
                                        Swal.fire({ icon: 'success', title: 'Approved!', text: res.message });
                                        location.reload();
                                    } else {
                                        Swal.fire('Error', res.message, 'error');
                                    }
                ...
                $(document).on('click', '#btnLock', function() {
                    ...
                    Swal.fire({
                        ...
                        preConfirm: () => {
                            return $.ajax({
                                url: '{{ route("payroll-runs.lock", ":id") }}'.replace(':id', id),
                                type: 'POST',
                                headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
                                success: function(res) {
                                    if (res.status === 'success') {
