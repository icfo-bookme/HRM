            ...
            <div class="flex gap-2">
                @if (in_array($loan->status, ['Pending', 'Rejected']))
                    <a href="{{ route('loan.edit', $loan->id) }}"
                        class="inline-flex items-center px-3 py-2 text-xs font-medium rounded-lg text-yellow-700 bg-yellow-100 hover:bg-yellow-200 transition-colors">
                    ...
                    <div class="p-5 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-indigo-50">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 rounded-lg bg-blue-600 flex items-center justify-center shadow-sm">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-800 mt-1">
                                    ...
                                    {{ $loan->employee->personalInfo?->full_name ?? 'N/A' }}</p>
                            </div>
                            <div>
                                <x-input-label value="Department" class="text-xs" />
                                <p class="text-sm font-medium text-gray-800 mt-1">
                                    {{ $loan->employee->department?->name ?? 'N/A' }}</p>
                            </div>
                            <div>
                                <x-input-label value="Designation" class="text-xs" />
                                <p class="text-sm font-medium text-gray-800 mt-1">
                                    {{ $loan->employee->designation?->title ?? 'N/A' }}</p>
                            </div>
                            <div>
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    <div class="p-5 border-b border-gray-100 bg-gradient-to-r from-purple-50 to-pink-50">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 rounded-lg bg-purple-600 flex items-center justify-center shadow-sm">
                                    <i class="fas fa-calendar-check text-white text-sm"></i>
                                </div>
                                <div>
                                    <h2 class="text-sm font-semibold text-gray-800">Installment Schedule</h2>
                                    <p class="text-xs text-gray-500">
                                        {{ $summary['paid_installments'] }}/{{ $summary['total_installments'] }} Paid
                                        @if (($summary['progress_installments'] ?? 0) > 0)
                                            &middot; {{ $summary['progress_installments'] }} In Progress
                                        @endif
                                    </p>
                                </div>
                            </div>
                        ...
                        <table class="w-full text-sm">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Amount
                                    </th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Due Date
                                    </th>
                                    <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Paid
                                    </th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Status
                                    </th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Payroll
                                        Run</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Paid At
                                    </th>
                                </tr>
                            </thead>
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div class="p-4 space-y-3">
                        <div class="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg">
                            ...
                            <span
                                class="text-xs font-bold text-gray-800">{{ number_format($loan->loan_amount, 2) }}</span>
                        </div>
                        <div class="flex items-center justify-between py-2 px-3 bg-orange-50 rounded-lg">
                            ...
                            <span
                                class="text-xs font-bold text-orange-600">{{ number_format($loan->total_interest, 2) }}</span>
                        </div>
                        <div class="border-t border-gray-100 pt-2">
                            <div class="flex items-center justify-between py-2 px-3 bg-blue-50 rounded-lg">
                                ...
                                <span
                                    class="text-xs font-bold text-blue-700">{{ number_format($loan->total_payable, 2) }}</span>
                            </div>
                            <div class="flex items-center justify-between py-2 px-3 mt-1.5 bg-indigo-50 rounded-lg">
                                ...
                                <span
                                    class="text-xs font-bold text-indigo-700">{{ number_format($loan->installment_amount, 2) }}</span>
                            </div>
                        ...
                        <div class="border-t border-gray-100 pt-2">
                            <div class="flex items-center justify-between py-2 px-3 bg-emerald-50 rounded-lg">
                                ...
                                <span
                                    class="text-xs font-bold text-green-700">{{ number_format($summary['total_paid'], 2) }}</span>
                            </div>
                            <div class="flex items-center justify-between py-2 px-3 mt-1.5 bg-purple-50 rounded-lg">
                                <span class="text-xs text-gray-600">In Progress:</span>
                                <span
                                    class="text-xs font-bold text-purple-700">{{ number_format($summary['total_progress'] ?? 0, 2) }}</span>
                            </div>
                            <div class="flex items-center justify-between py-2 px-3 mt-1.5 bg-red-50 rounded-lg">
                                ...
                                <span
                                    class="text-xs font-bold text-red-700">{{ number_format($summary['total_pending'], 2) }}</span>
                            </div>
                        </div>
                        ...
                        <div class="pt-2">
                            <div class="flex items-center justify-between mb-1.5">
                                ...
                                <span
                                    class="text-xs text-gray-600">{{ $summary['paid_installments'] }}/{{ $summary['total_installments'] }}
                                    Paid</span>
                            </div>
                            <div class="w-full bg-gray-200 rounded-full h-2.5 flex">
                                @php $paidPercent = $percent; @endphp
                                @php $progressPercent = $summary['progress_installments'] > 0 ? round(($summary['progress_installments'] / max($loan->total_installments, 1)) * 100) : 0; @endphp
                                <div class="h-2.5 rounded-l-full transition-all duration-500 bg-green-500"
                                    style="width: {{ $paidPercent }}%"></div>
                                @if ($progressPercent > 0)
                                    <div class="h-2.5 transition-all duration-500 bg-purple-400"
                            ...
                            <div class="flex justify-between text-xs text-gray-500 mt-1">
                                ...
                                @if ($progressPercent > 0)
                                    <span>{{ $progressPercent }}% In Progress</span>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    @if ($loan->status === 'Pending')
                        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                            <div class="p-4 border-b border-gray-100 bg-gradient-to-r from-amber-50 to-orange-50">
                                <div class="flex items-center gap-2.5">
                                    <div
                                        class="w-8 h-8 rounded-lg bg-amber-600 flex items-center justify-center shadow-sm">
