...
<div
    class="document-row bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-200">
    <div
        class="bg-gradient-to-r from-slate-50 to-white px-6 py-3.5 border-b border-slate-100 flex items-center justify-between">
        ...
        <button type="button"
            class="remove-document inline-flex items-center gap-1.5 text-xs font-medium text-red-500 hover:text-red-700 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors">
    ...
    <div class="p-6 space-y-5">
        <div class="grid gap-5 sm:grid-cols-2">
            ...
            <div>
                <x-form-input label="Document Name" name="documents[{{ $index }}][document_name]"
                    placeholder="e.g. National ID Card" :value="old(\"documents.{$index}.document_name\",
        ...
        <div>
            ...
            <div class="file-info mt-2 {{ empty($document['file_path']) ? 'hidden' : '' }}">
                <div class="flex items-center gap-2 text-xs text-emerald-600 bg-emerald-50 rounded-lg px-3 py-2">
                    ...
                    <span
                        class="file-name">{{ !empty($document['file_path']) ? basename($document['file_path']) : '' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            <x-form-input label="Document Number" name="documents[{{ $index }}][document_number]"
                placeholder="Reference number (optional)" :value="old(\"documents.{$index}.document_number\",
                $document['document_number'] ?? '' )" />
            <x-form-input label="Issuing Authority" name="documents[{{ $index }}][issuing_authority]"
                placeholder="Issuer name (optional)" :value="old(\"documents.{$index}.issuing_authority\",
