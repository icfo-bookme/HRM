    ...
    <div class="p-4 max-w-4xl mx-auto">
        <div class="bg-white border border-slate-200 shadow-sm rounded-3xl p-8">
            <div class="mb-8 flex items-start justify-between">
                <div>
                    <h1 class="text-3xl font-semibold text-slate-900">Create New Notice</h1>
                    <p class="mt-2 text-sm text-slate-600">Fill in the details below to publish a new notice.</p>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('notice.store.page') }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div class="grid gap-6 sm:grid-cols-2">
                    <x-form-select label="Notice Type" name="notice_type" placeholder="Select Type" required>
                        @foreach ($noticeTypes as $value => $label)
                            <option value="{{ $value }}" {{ old('notice_type') === $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </x-form-select>
                ...
                <div class="grid gap-6 sm:grid-cols-2">
                    <x-form-select label="Target Type" name="target_type" placeholder="Select Target" required>
                        @foreach (['All', 'Department', 'Designation', 'Branch', 'Employee'] as $type)
                            <option value="{{ $type }}" {{ old('target_type', 'All') === $type ? 'selected' : '' }}>{{ $type }}</option>
                        @endforeach
                    </x-form-select>
                ...
                <div class="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                    <label class="flex items-center gap-2 cursor-pointer select-none">
                        ...
                        <input type="checkbox" name="is_popup" value="1" {{ old('is_popup') ? 'checked' : '' }}
                ...
                <div class="flex justify-between items-center pt-4 border-t border-slate-200">
                    ...
                    <button type="submit"
                        class="inline-flex items-center justify-center rounded-2xl bg-blue-800 px-6 py-3 text-sm font-semibold text-white transition hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-bg-blue-900 focus:ring-offset-2">
                        Publish Notice
                    </button>
                </div>
            </form>
