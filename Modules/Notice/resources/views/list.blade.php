            ...
            @else
                <div class="space-y-4">
                    @foreach($notices as $notice)
                        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-all {{ $notice->is_pinned ? 'ring-2 ring-blue-500 ring-offset-1' : '' }}">
                            {{-- Pinned Badge --}}
                            @if($notice->is_pinned)
                                <div class="bg-gradient-to-r from-blue-700 via-blue-900 to-blue-950 px-4 py-1 flex items-center gap-1.5">
                            ...
                            <div class="p-6">
                                ...
                                {{-- Footer: Attachment + Acknowledgment Status + Detail Link --}}
                                <div class="flex items-center justify-between flex-wrap gap-3 pt-3 border-t border-slate-100">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        @php
                                            $userAck = $notice->acknowledgements->first();
                                        @endphp
                                        @if($userAck)
                                        ...
                                        @endif
                                        @if($notice->views_count > 0)
                                            <span class="text-xs text-gray-400">
                                                <i class="fas fa-eye mr-1"></i>{{ $notice->views_count }} {{ Str::plural('seen', $notice->views_count) }}
                                            </span>
                                        @endif
