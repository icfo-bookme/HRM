<x-app-layout>
    <div class="p-4 lg:p-6">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('loan.my') }}" class="inline-flex items-center px-3 py-2 text-xs font-medium rounded-lg text-gray-700 bg-gray-100 hover:bg-gray-200 transition-colors">
                    ...
                    <div class="p-5 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-indigo-50">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">Fill in the details below to submit your loan request</p>
                            </div>
                        </div>
                    </div>
                    <div class="p-6">
                        <form id="loanForm" action="{{ route('loan.store') }}" method="POST">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-2.5 w-full border border-gray-200 rounded-lg px-3.5 py-2.5 bg-gray-50 mt-1.5">
                                        <i class="fas fa-user text-gray-400 text-xs"></i>
                                        <span class="text-xs text-gray-600 font-medium">{{ $employee->employee_code }} - {{ $employee->personalInfo?->full_name ?? 'N/A' }}</span>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="flex items-center gap-2.5 w-full border border-gray-200 rounded-lg px-3.5 py-2.5 bg-gray-50 mt-1.5">
                                        ...
                                        <span class="text-xs text-gray-600 font-medium">{{ $employee->department?->name ?? 'N/A' }}</span>
                                    </div>
                                </div>

                                <!-- Loan Type (Custom Select with chevron icon) -->
                                <div>
                                    <x-input-label for="loan_type" value="Loan Type" class="text-xs font-semibold" :required="true" />
                                ...
                                <div>
                                    <x-input-label for="loan_amount" value="Loan Amount" class="text-xs font-semibold" :required="true" />
                                    <div class="relative mt-1.5">
                                        <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                            <span class="text-gray-400 text-xs font-medium">$</span>
                                        </div>
                                        <input type="number" name="loan_amount" id="loan_amount" step="0.01" min="1" required
                                ...
                                <div>
                                    <x-input-label for="interest_rate" value="Interest Rate (%)" class="text-xs font-semibold" />
                                    <div class="relative mt-1.5">
                                        <input type="number" name="interest_rate" id="interest_rate" step="0.01" min="0" max="100" value="0"
                                        ...
                                        <span class="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs font-medium">%</span>
                                    </div>
                                ...
                                <div>
                                    <x-input-label for="total_installments" value="Installments (Months)" class="text-xs font-semibold" :required="true" />
                                    <div class="relative mt-1.5">
                                        <select name="total_installments" id="total_installments" required
                                            ...
                                            @for($i = 1; $i <= 60; $i++)
                                                <option value="{{ $i }}" {{ $i == 12 ? 'selected' : '' }}>{{ $i }} {{ $i === 1 ? 'Month' : 'Months' }}</option>
                                            @endfor
                                        </select>
                                        <i class="fas fa-chevron-down absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs pointer-events-none"></i>
                                    </div>
                            ...
                            <!-- Notes -->
                            <div class="mt-4">
                                <x-input-label for="notes" value="Additional Notes" class="text-xs font-semibold" />
                            ...
                            <div class="mt-6 pt-5 border-t border-gray-100 flex items-center gap-3">
                                <button type="submit"
                                    class="inline-flex items-center gap-2 px-5 py-2.5 text-xs font-semibold rounded-lg text-white bg-blue-600 hover:bg-blue-700 focus:ring-2 focus:ring-blue-500/30 transition-all shadow-sm">
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        <div class="flex items-center justify-between py-2 px-3 bg-indigo-50 rounded-lg">
                            ...
                            <span class="text-xs font-bold text-blue-700">{{ number_format($loanSummary['monthly_deduction'], 2) }}</span>
                        </div>
                        <div class="flex items-center justify-between py-2 px-3 bg-violet-50 rounded-lg">
                            ...
                            <span class="text-xs font-bold text-purple-700">{{ number_format($loanSummary['total_remaining'], 2) }}</span>
                        </div>
                        <div class="flex items-center justify-between py-2 px-3 bg-emerald-50 rounded-lg">
                            ...
                            <span class="text-xs font-bold text-green-700">{{ number_format($loanSummary['total_paid'], 2) }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-4" id="loan-preview">
                        ...
                        <div id="preview-details" class="hidden space-y-3">
                            ...
                            <div class="border-t border-gray-100 pt-2">
                                ...
                                <div class="flex items-center justify-between py-2 px-3 mt-1.5 bg-emerald-50 rounded-lg">
                                    <span class="text-xs font-semibold text-gray-700">Per Installment:</span>
                                    <span class="text-xs font-bold text-emerald-700" id="preview-installment">0.00</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            function calculateLoan() {
                ...
                $.ajax({
                    ...
                    error: function() {
                        ...
                        let perInstallment = installments > 0 ? totalPayable / installments : totalPayable;
